...
var SuggestedBigItem = function (documentProps, itemProps) {
    ...
    return (React.createElement(Persona_1.Persona, { imageUrl: (_a = documentPreviewProps) === null || _a === void 0 ? void 0 : _a.previewImages[0].previewImageSrc, text: documentTitleProps && documentTitleProps.title, size: Persona_1.PersonaSize.size40 }));
};
...
var SelectedDocumentItem = function (documentProps) {
    ...
    var actions = documentActionsProps
        ? tslib_1.__spreadArrays(documentActionsProps.actions, [
            {
...
var getTextFromItem = function (props) {
    return props.documentTitleProps.title;
};
var listContainsDocument = function (document, items) {
    if (!items || !items.length || items.length === 0) {
        return false;
    }
    var documentTitle = document.documentTitleProps && document.documentTitleProps.title;
    return items.some(function (item) { return (item.documentTitleProps && item.documentTitleProps.title) === documentTitle; });
};
var onFilterChanged = function (filterText, items) {
    if (!items) {
        return [];
    }
    return filterText
        ? data
            ...
            .filter(function (item) { return !listContainsDocument(item, items); })
        : [];
};
exports.PickerCustomResultExample = function () {
    var _a = react_hooks_1.useBoolean(false), isPickerDisabled = _a[0], toggleIsPickerDisabled = _a[1].toggle;
    return (React.createElement("div", { className: rootClass },
        React.createElement(Toggle_1.Toggle, { styles: toggleStyles, label: "Disable document picker", checked: isPickerDisabled, onChange: toggleIsPickerDisabled }),
        React.createElement(DocumentPicker, { removeButtonAriaLabel: "Remove", onRenderSuggestionsItem: SuggestedBigItem, onResolveSuggestions: onFilterChanged, onRenderItem: SelectedDocumentItem, getTextFromItem: getTextFromItem, pickerSuggestionsProps: pickerSuggestionsProps, disabled: isPickerDisabled, inputProps: inputProps })));
};
